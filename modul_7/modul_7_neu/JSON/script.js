...
function renderPosts(books) {
    let contentRef = document.getElementById('book_post');
    contentRef.innerHTML = "";

    for (let indexPost = 0; indexPost < books.name.length; indexPost++) {
        contentRef.innerHTML += getPostTemplate(indexPost);
        
    };
}

function commentSection(books.comments[0]) {
    let comments = document.getElementById('comment_bar');
}

function addComment() {
    let commentInputRef = document.getElementById('comment_input');
    //eingabe auslesen
...
//kommentar löschen
function getPostTemplate(indexPost) {
    return `
    <div class="post">
        <h3>+ ${books.name[indexPost]}</h3>
        <p>${books.author[indexPost]}</p>
        <div>
            <button onclick="addLike(${indexPost}, 'notes', 'trashNotes')" class="btn">X</button>
            <input id="write_comment" class="comment_bar" type="text">
        </div>
    </div>`;
}